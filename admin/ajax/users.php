...
if (isset($_POST['get_users'])) {
    ...
    {
        $del_btn = "
            <button type='button' onclick='remove_user($row[id])' class='btn btn-danger shadow-none btn-sm mb-1'>
                <i class='bi bi-trash'></i>
            </button>
        ";
        $verified = "<span><i class='bi bi-x-octagon-fill text-danger fs-3'></i></span>";

        if($row['is_verified']){
            $verified = "<span><i class='bi bi-patch-check-fill text-info fs-3'></i></span>";
            $del_btn = "";
        }
